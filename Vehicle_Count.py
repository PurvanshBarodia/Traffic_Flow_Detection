...
# import all deep sort carpet files
...
nn_budget = None
nms_max_overlap = 1.0
# initialization of deep sort parameters
...
# function to get the names of the levels of the neutron network
...
# function to define the color of each class, this function works with the classeIDs that are recognized,
#that send a value for the respective class, 0,1,2,3 ..n classes. Look at the coconut file, which has the objects organized.
...
# ## Functions to define the intersection between the counting lines

# In[7]:


# functions to determine and/or find the equation of the line from two points and verify if there is an intercept
...
def calcParams(point1, point2): #line's equation Params computation, receives the past and current centroid
    if point2[1] - point1[1] == 0:
        ...
        a = (point2[1] - point1[1]) / (point2[0] - point1[0]) # this seems to be the slope m=(y2-y1)/(x2-x1)
        b = -1.0

    c = (-a * point1[0]) - b * point1[1] # General Equation ax+by+c=0, if I clear c= -ax-by
                                          # model point slope y-y1 = m(x-x1)
    return Params(a,b,c)
...
# the position of the counting line is defined within the ROI and the abc parameters of that line are found for later
# verify if the consecutive centroids of the objects are being interpreted in it, IT IS ONLY DONE ONCE
pos_linha1 = 226
...
primer_frame=False # initialize first frame to choose RoI
...
#                          ],dtype=np.int32)
# x=cv2.fillPoly(mask,[vertices],(255,255,255)) # x is the rgb mask that contains the original size of the input image 
                                                # only that shows ROI and the other pixels in black
...
# # create .txt with the count by regions
...
# ## Functions to determine the movement through the highway

# In[12]:


def regionesEntrada(lisIdsIdentificados,colums,lisIdsEliminados,regiones,frame_numero,TT,lisIdsNoIdentificados):
    # this for eliminates the ides already identified by ids, that is, the dataframe columns that are associated with each vehicle
    for i in lisIdsIdentificados: # if i=0 is in that list, that value will be removed
        colums.remove(i)
    
    for i in lisIdsEliminados:
        colums.remove(i)

    for i in colums: #0 1 3  # This contains the number of vehicles detected at the moment they are in the dataframe.
        centroCuestion = df.iloc[frame_numero][i] # the centers of the current detections of the dataframe are accessed
